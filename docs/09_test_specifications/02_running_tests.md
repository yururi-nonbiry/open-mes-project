...
```
docs/09_test_specifications/reports/inventory.md
```

ファイル名は `<第1引数を _ 区切りにしたもの>.md` の固定パスであり、実行するたびに同一ファイルへ
上書き保存される（タイムスタンプ付きの新規ファイルは追加しない）。過去の実行結果の履歴が必要な場合は
`git log -p` でこのファイルの変更履歴を辿ればよく、レポートの世代管理をファイル名側で行う必要はない。
レポートは通常のファイルなので、`git add`/`git commit` で他のドキュメントと同様にコミットできる。
