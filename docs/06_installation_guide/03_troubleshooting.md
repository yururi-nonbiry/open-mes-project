...
- **Docker関連の問題**:
  Windows環境でDocker Desktopを使用している場合、WSL2の設定やリソース制限によってコンテナが不安定になることがあります。Docker Desktopの設定でメモリ/CPUの割当を増やす、WSLの更新を行う、あるいはUbuntu上で直接Dockerを動かすなどの対策を試してください。Dockerを使わない場合は`start.bat`（Windows）によるローカルセットアップも利用できます（[セットアップ手順](./02_setup.md)を参照）。
